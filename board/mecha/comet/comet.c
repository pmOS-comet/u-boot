...
struct udevice *btn;
	...
	env_set("board_rev", "iMX8MP-rev1");
	env_set("stdin", "serial,button-kbd");
#endif

	int ret = button_get_by_label("up", &btn);
	if (!ret && button_get_state(btn) == 0) {
		printf("Volume Up held: entering Expo\n");
		env_set("boot_expo", "1");
		run_command("cedit load mmc 2:1 expo_layout.dtb", 0);
		int ret;

		ret = cedit_run(cur_exp);
		cedit_write_settings_env(cur_exp, true);
		ulong boot;
		env_get_ulong("c.dev-menu-value", 10, boot);	
		printf("Value: %ld\n", boot);

		if (ret == -EACCES) {
			/* User pressed Exit / Home */
			printf("Expo exited by user\n");
			return 0;
		}

		if (ret) {
			printf("Expo failed (err=%d)\n", ret);
			return ret;
		}
	}
	return 0;
